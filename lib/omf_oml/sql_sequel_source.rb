...
module OMF::OML
  ...
  class OmlSqlSource < OMF::Base::LObject
    ...
    def initialize(db_opts, row_opts = {})
      ...
      @tables = {}
      @db_opts = db_opts
      if (@db = @@db_connections[db_opts])
        puts ">>>>>>>>>>> REUSING"
        debug "Reusing existing DB (#{db_opts})"
      else
        debug "Opening DB (#{db_opts})"
        if db_opts.is_a? Hash
          db_opts[:max_connections] ||= 10
          @db = Sequel.connect(db_opts)
        else
          @db = Sequel.connect(db_opts, max_connections: 10)
        end
         @@db_connections[db_opts] = @db if @db
      end
      debug "DB: #{@db.inspect}"
      @row_opts = row_opts
    end
